...
from pwassist.io.binning import BinBundle, BinCollection, KinematicBin, MassBin
from pwassist.io.catalog import Catalog, DataFile, FitFile

# TODO: Add tests for MassBin, TBin, EnergyBin, and KinematicBin classes
# class TestMassBin:
...
def sample_catalog(tmp_path: Path) -> Catalog:
    ...
    data2.write_text(
        "events,efficiency,m_low,m_high,e_low,e_high,t_low,t_high\n"
        "2000,0.10,1.10,1.20,8.2,8.8,0.35,1.0\n"
    )
...
class TestBinBundle:
    ...
    def test_bin_bundle_creation(self, sample_manifest):
        ...
        assert bundles[0].kinematic_bin.t_bin.low == 0.23
        assert bundles[0].kinematic_bin.t_bin.high == 0.35
        assert bundles[0].kinematic_bin.energy_bin.low == 8.2
        assert bundles[0].kinematic_bin.energy_bin.high == 8.8
        assert bundles[0].kinematic_bin.mass_bin.low == 1.0
        assert bundles[0].kinematic_bin.mass_bin.high == 1.1
        assert bundles[0].bin_id == "T=0.23,0.35-E=8.2,8.8-M=1.0,1.1"
...
class TestBinCollection:
    ...
    def test_from_manifest(self, sample_manifest):
        ...
        first_bin = KinematicBin.from_bin_id("T=0.23,0.35-E=8.2,8.8-M=1.0,1.1")
        second_bin = KinematicBin.from_bin_id("T=0.35,1.0-E=8.2,8.8-M=1.1,1.2")
        ...
        for kinematic_bin, bundle in collection:
            if kinematic_bin == first_bin:
                assert bundle.bin_id == "T=0.23,0.35-E=8.2,8.8-M=1.0,1.1"
            elif kinematic_bin == second_bin:
                assert bundle.bin_id == "T=0.35,1.0-E=8.2,8.8-M=1.1,1.2"
